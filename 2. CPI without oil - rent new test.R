...
plot(CPIs$Year, CPIs$`Index.without.housing.rental`, type = "l", col = "red", xlab = "Year", ylab = "Index", main = "1")
plot(CPIs$Year, CPIs$`Index.without.petroleum.products`, type = "l", col = "red", xlab = "Year", ylab = "Index", main = "2")

CPIs$Total2 <- CPIs$Total - 0.19*CPIs$Housing.rental.1
plot(CPIs$Year, CPIs$`Index.without.housing.rental`, type = "l", col = "red", xlab = "Year", ylab = "Index", main = "2")
plot(CPIs$Year, CPIs$Total2, type = "l", col = "red", xlab = "Year", ylab = "Index", main = "3")
plot(CPIs$Year, CPIs$Total, type = "l", col = "red", xlab = "Year", ylab = "Index", main = "4")

# add a new column to the data
CPIs$Index.withoutRI <- CPIs$`Index.without.petroleum.products` - 0.19325*CPIs$`Housing.rental`
...
CPIs$Inf_nooil <- log(CPIs$Index.without.petroleum.products/lag(CPIs$Index.without.petroleum.products,12))
CPIs$Inf_rent <- log(CPIs$Rent/lag(CPIs$Rent,12))
CPIs$New <- CPIs$Inf_nooil- 0.19325*CPIs$Inf_rent
plot(CPIs$Year, CPIs$New, type = "l", col = "red", xlab = "Year", ylab = "Inflation rate", main = "1")
plot(CPIs$Year, CPIs$Inflation.withoutRI_log, type = "l", col = "red", xlab = "Year", ylab = "Inflation rate", main = "2")

CPIs <- CPIs[13:nrow(CPIs),]
CPIs$New <- CPIs$New - lag(CPIs$New,1)
CPIs <- CPIs[2:nrow(CPIs),]
# Calcul de la transformation de Fourier rapide (FFT)
fft_result <- fft(CPIs$New)

# Calcul des fréquences
frequencies <- seq(from = 0, to = length(CPIs$New)/2, length.out = length(CPIs$New)/2 + 1)
...
## NOT stationnary
plot(CPIs$New)
auto <- auto.arima(CPIs$New, seasonal = FALSE, approximation = FALSE, trace=TRUE)
